  ...
  static setTileLayer(mapId: number, theme: AppTheme, map: L.Map) {
    ...
    switch (mapId) {
      ...
      case 3: {
        ...
        // то в адресе GisApi надо явно указать порт localhost:5289
        ...
        // либо localhost если браузер на той же машине где сервер

        // GisApi сервис слушает только порт http: :5289
        // и запрос снаружи  маршрутится туда nginx'ом
        // идет https запрос это 443 порт, а дальше там стоит перенаправление
        //     upstream gis-api {
        //        server localhost:5289;
        //     }
        //     location /gis/ {
        //        #proxy_pass https://localhost:7151; раньше было но на дебиане не заработало
        //        proxy_pass http://gis-api; всё что идет на адрес/gis перенаправлять на upstream gis-api
        //     }
