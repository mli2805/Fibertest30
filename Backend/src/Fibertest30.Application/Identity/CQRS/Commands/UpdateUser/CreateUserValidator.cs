...
public class CreateUserValidator : AbstractValidator<CreateUserCommand>
{
    public CreateUserValidator()
    {
        ...
        RuleFor(x => x.Patch.UserName).NotNull().NotEmpty().MaximumLength(100);
        RuleFor(x => x.Patch.FirstName).NotNull().NotEmpty().MaximumLength(100);
        RuleFor(x => x.Patch.Email).NotNull().NotEmpty().EmailAddress();
